        ...
        <div className="forest__diary-date-container">
          {getDaysArray().map((day, i) => (
            <div
              key={i}
              id={getIdForDate(day)}
              className={`forest__diary-date-round ${day.toDateString() === createdAt.toDateString() ? 'forest__diary-date-round--today' : ''}`}
      ...
      <div className="forest__diary-actions">
        <div className="forest__diary-actions-reveal">
          {['only', 'subscribe', 'all'].map((className, index) => (
            <div
              key={index}
              className={`forest__diary-actions-reveal-btn forest__diary-actions-reveal-btn--${className} ${access_level === index ? 'forest__diary-actions-reveal-btn--selected' : ''}`}
